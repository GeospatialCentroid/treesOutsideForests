###############################################################################
# Script for classified map generatoin from input 4-band NAIP imagery using a 
# trained U-Net model.
###############################################################################
...
stride = 200
inf_size = 384
cell_years = ['X12-318_2010']
input_naip_files_path = Path(f'D:/Shahriar/Work_Datasets/Agroforestry/NAIP_download/tiles')
trained_model_path = Path('D:/Shahriar/Work_Datasets/Agroforestry/Unet_test/training_outputs/20251230/unet_classification_104341.keras')
output_path = Path(f'D:/Shahriar/Work_Datasets/Agroforestry/Unet_test/test_more_grids/classified_maps')
naip_bands = 4
...
for chosen_cell_year in cell_years:

    input_image_folder = (input_naip_files_path / f'{chosen_cell_year}').glob(f'{chosen_cell_year}_naip_tile*.tif')
    output_folder = output_path / f'{model_timestamp}'
    os.makedirs(output_folder, exist_ok=True)

    for image in input_image_folder:
        ...
        map = sliding_window_predict(model, img, best_threshold,
                                          ...
                                          inf_size=384,
                                          stride=200,
                                          batch_size=1)  # try 1,2,4,8 depending on GPU mem

        test_image_name = image.stem
        output_path = output_folder / test_image_name.replace('mosaic','pred')
        ...
        with rasterio.open(
            ...
            dst.write(map, 1)
        print("✅ Stitched prediction saved to {}. Elapsed time was {:.0f} seconds.".format(output_path, time.time()-start_time))
