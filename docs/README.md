...
GitHub Pages serves the `docs/` folder of the `main` branch as a static
website. A small R script copies the finished maps from `data/sampling/maps/`
into `docs/maps/` and writes a `docs/index.html` gallery that links to them.
Building the maps needs the NAS data, so the build runs locally; committing
`docs/` and pushing is the deploy. No GitHub Actions, no Jekyll, no framework.
The site is a set of plain files that any web server or a local browser can
open, which is what makes it robust.
...
| Plain generated HTML index | No site generator to install or upgrade. `pandoc` and `quarto` are not on this machine today. |
| `.nojekyll` file in `docs/` | Turns off GitHub's Jekyll pass, which is slower and can drop folders whose names start with an underscore. |
...
- **A GitHub Action that only checks**, never builds: run a link checker on
  `docs/` on each push so a missing sidecar folder fails loudly. Worth adding
  once more than one person is committing maps.
